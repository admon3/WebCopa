...
module.exports = function(Multimedia) {
    Multimedia.uploadImage = function(id, req, res, cb) {
        Multimedia.findById(id, function(err, instance) {
          ...
          storageService.upload(req, res, {container: 'multimedia'}, function(err, fileObject) {
              if (err) {
                ...
                instance.updateAttributes({
                    ruta: `/api/containers/multimedia/download/${fileObject.files.file[0].name}`,
                    tipo: fileObject.files.file[0].type
